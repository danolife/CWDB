...
import { wordsTable } from '../src/domains/db/schema.ts';
...
async function main() {
  ...
  try {
    const wordsFiles = await listFiles(/^words_\d+\.txt$/);

    for (const file of wordsFiles) {
      await processFileByLines(file, async (words) => {
        await db.insert(wordsTable).values(words.map((word) => ({ word })));
        console.log(`Inserted ${words.length} words from ${file}`);
      });
    }
  } catch (error) {
    console.error(error);
